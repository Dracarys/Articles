...
可以通&&和||来组合判断条件，通过！做逻辑假判断，还可以通过#elseif和#else来添加条件分支，此外还可以在一个选择编译Block中嵌套另一个Block。
...
与C语言中的预编译不同，Swift中的条件编译block必须完整且语法正确，这是因为所有的SWift代码都会进行语法检查，即使尚未被编译。
但有一个例外，如果条件编译包含了swift()判断，那么这个表达式仅在Swift版本与判断条件相匹配的时候在会去解析该表达式。这个例外是为了确保旧版编译起不会去尝试解析由更新的Swift语法。
